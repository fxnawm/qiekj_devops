...
class OssSdk:
    ...
    def upload(self):
        ...
        bucket.put_object_from_file('soft/xyj_xyy_day.xlsx', self.path)
...
class Excel:
    def __init__(self,select_sql,ysj_sql):
        self.select_sql = select_sql
        self.ysj_sql = ysj_sql
    ...
    def main(self):
        ...
        for x in data:
            ...
            wb = load_workbook("xyj_xyy_day.xlsx")
            ...
            wb.save('xyj_xyy_day.xlsx')
    def ysj(self):
        client = MySQL()
        data = client.query_tuple(self.ysj_sql)
        for x in data:
            data = ["%.2f%%" % (i * 100) if isinstance(i, float) and i < 1 else i for i in x]
            wb = load_workbook("xyj_xyy_day.xlsx")
...
if __name__ == '__main__':
    ...
    #     insert_sql = f.read()
    with open('xyy_day.sql',encoding='utf-8') as f:
        xyy_sql = f.read()
    with open('ysj_day.sql', encoding='utf-8') as f:
        ysj_sql = f.read()
    Action = Excel(xyy_sql,ysj_sql)
    Action.main()
    Action.ysj()
    ossclient = OssSdk('xyj_xyy_day.xlsx')
    ossclient.upload()