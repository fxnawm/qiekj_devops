...
class OssSdk:
	...
	def upload(self):
		...
		bucket.put_object_from_file('soft/merchant_yunying_weekday.xlsx', self.path)
...
class Excel:
	...
	def main(self):
		...
		print(["%.2f%%" % (i * 100) if isinstance(i, float) and i < -10000000 else i for i in data])



if __name__ == '__main__':
	# with open('merchant_yunying_weekday.sql',encoding='utf-8') as f:
	# 	insert_sql = f.read()
	# with open('merchant_yunying_weekday_new.sql',encoding='utf-8') as f:
	# 	insert_weekday = f.read()
	with open('merchant_yunying_weekday_select.sql',encoding='utf-8') as f:
		select_sql = f.read()
	Action = Excel(select_sql)
	Action.main()
	#ossclient = OssSdk('merchant_yunying_weekday.xlsx')
	#ossclient.upload()
